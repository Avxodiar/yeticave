<form class="form form--add-lot container <?=$hasValidError ? 'form--invalid' : ''?>" method="post" enctype="multipart/form-data">
    <h2>Добавление лота</h2>

    <div class="form__container-two">
        <div class="form__item <?=$hasFieldValidError['lot-name'] ? 'form__item--invalid' : ''?>">
        ...
        <div class="form__item <?=$hasFieldValidError['category'] ? 'form__item--invalid' : ''?>">
    ...
    <div class="form__item form__item--wide <?=$hasFieldValidError['message'] ? 'form__item--invalid' : ''?>">
    ...
    <div class="form__container-three">
        <div class="form__item form__item--small <?=$hasFieldValidError['lot-rate'] ? 'form__item--invalid' : ''?>">
            <label for="lot-rate">Начальная цена</label>
            <input id="lot-rate" type="number" name="lot-rate" placeholder="0" required value="<?=$arRes['lot-rate']?>">
            <span class="form__error">Введите начальную цену</span>
        </div>
        <div class="form__item form__item--small <?=$hasFieldValidError['lot-step'] ? 'form__item--invalid' : ''?>">
            <label for="lot-step">Шаг ставки</label>
            <input id="lot-step" type="number" name="lot-step" placeholder="0" required value="<?=$arRes['lot-step']?>">
            <span class="form__error">Введите шаг ставки</span>
        </div>
        <div class="form__item <?=$hasFieldValidError['lot-date'] ? 'form__item--invalid' : ''?>">
            <label for="lot-date">Дата окончания торгов</label>
            <input class="form__input-date" id="lot-date" type="date" name="lot-date" required value="<?=$arRes['lot-date']?>">
            <span class="form__error">Введите дату завершения торгов</span>
        </div>
    </div>
    <span class="form__error <?=$hasValidError ? 'form__error--bottom' : ''?>">Пожалуйста, исправьте ошибки в форме.</span>
